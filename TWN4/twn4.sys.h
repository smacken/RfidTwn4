...
#define LFTAG_EM4305        0x48    // N/A                 (aka IDRW_K)
...
#define BLE_EVENT_GATT_MTU_EXCHANGED 				0x45
#define BLE_EVENT_GATT_SERVER_ATTRIBUTE_VALUE 		0x51
#define BLE_EVENT_GATT_SERVER_CHARACTERISTIC_STATUS 0x52
#define BLE_EVENT_GATT_CHARACTERISTIC 				0x53
#define BLE_EVENT_GATT_CHARACTERISTIC_VALUE 		0x54
#define BLE_EVENT_GATT_SERVICE 						0x55
#define BLE_EVENT_GATT_PROCEDURE_COMPLETED 			0x56
#define BLE_EVENT_GATT_SERVER_USER_READ_REQUEST		0x57
#define BLE_EVENT_GATT_SERVER_USER_WRITE_REQUEST	0x58
#define BLE_EVENT_GATT_SERVER_EXECUTE_WRITE_COMPLETED 0x59
...
#define BLE_EVENT_CONNECTION_PHY_STATUS				0x75
#define BLE_EVENT_LE_GAP_SCAN_RESPONSE 				0x81
#define BLE_EVENT_LE_GAP_EXTENDED_SCAN_RESPONSE		0x82
#define BLE_EVENT_SM_BONDED 						0x91
#define BLE_EVENT_SM_BONDING_FAILED 				0x92
#define BLE_EVENT_SM_LIST_ALL_BONDINGS_COMPLETE		0x93
#define BLE_EVENT_SM_LIST_BONDING_ENTRY				0x94
#define BLE_EVENT_SM_PASSKEY_DISPLAY 				0x95
#define BLE_EVENT_SM_PASSKEY_REQUEST 				0x96
#define BLE_EVENT_SM_CONFIRM_BONDING 				0x99
#define BLE_EVENT_SM_CONFIRM_PASSKEY 				0x9A
#define BLE_EVENT_SYSTEM_BOOT 						0xA2
#define BLE_EVENT_DTM_COMPLETED 					0xF0
...
// ******************************************************************
// ****** TCSP (Timing-Critical Serial Protocol) ********************
// ******************************************************************

#ifndef __TWN4_SYS_C__

// Supported Standard
#define TCSP_OSDP_STANDARD_V2_1_7       0x01

// RX/TX Buffer Size Restrictions
#define TCSP_MIN_RX_PACKET_SIZE         128         // OSDP Standard Limit: must be able to receive at least this many bytes
#define TCSP_MAX_PACKET_SIZE            1440        // OSDP Standard Limit: absolute maximum packet size (in bytes) that can be sent/received
#define TCSP_MIN_TX_DATA_BUFFER_SIZE    128         // Elatec-imposed constraint: to make sure packets sent by FW have no issues
#define TCSP_CARD_DATA_BUFFER_SIZE      64          // Transponder ID cannot be longer than this many bytes

// TCSP Link-up with Host States
#define TCSP_LINK_INACTIVE              false
#define TCSP_LINK_ACTIVE                true

// TCSP Serial data Rates
#define TCSP_DATA_RATE_9600             9600
#define TCSP_DATA_RATE_19200            19200
#define TCSP_DATA_RATE_38400            38400
#define TCSP_DATA_RATE_76800            76800

// Check Type
#define TCSP_CHECKSUM                   0x00
#define TCSP_CRC                        0x01

// Power, Tamper, Reader StatusWord
#define TCSP_TAMPER_STATUS_OK           0x00
#define TCSP_TAMPER_STATUS_BAD          0x01
#define TCSP_TAMPER_STATUS_NO_CHANGE    0x02
#define TCSP_TAMPER_STATUS_INVALID      0x04

#define TCSP_POWER_STATUS_OK            0x00
#define TCSP_POWER_STATUS_BAD           0x01
#define TCSP_POWER_STATUS_NO_CHANGE     0x02
#define TCSP_POWER_STATUS_INVALID       0x04

// "TCSP_ReceivePacket" return codes
#define TCSP_NO_NEW_DATA_AVAILABLE      0x00    // Nothing for App to process.
#define TCSP_NEW_DATA_RETRIEVED         0x01    // Entire new TCSP packet received.
#define TCSP_ERROR_RX_PACKET_TOO_LONG   0x02    // TCSP packet cannot be received, because it is bigger than RxData array.
                                                // Increase RxData Array size in App.

// "TCSP_SendPacket" return codes
#define TCSP_TX_DATA_SUBMITTED          0x80    // Data Packet will be sent at the next Controller Request.
#define TCSP_TX_DATA_TOO_LONG           0x81    // Data Packet too long to be sent.
#define TCSP_COMMAND_NO_LONGER_VALID    0x82    // Controller now requesting a different command.

// Priority settings to arbitrate between Firmware and App requests
#define TCSP_PRIORITY_NO_ACTION         0x00    // These settings will be ignored
#define TCSP_PRIORITY_RECESSIVE         0x01    // Use in App to set "weak" settings that Firmware can later change
#define TCSP_PRIORITY_NEUTRAL           0x02    // Firmware typically operates at this level
#define TCSP_PRIORITY_DOMINANT          0x04    // Use in App to prevent Firmware from overriding

#define TCSP_PRIORITY_UPDATE            0x80    // Flag to Update Priority level, not just read it
                                                // If this is set, then settings take effect

// Passing configuration to LED
typedef struct __attribute__((__packed__))
{
    uint8_t     Reader;
    uint8_t     ID;
    uint8_t     TempPriority;
    uint32_t    TempStateCount;     // if 0, then permanent state active immediately, otherwise temporary state
    uint16_t    TempOnTime;         // maximum value can be 255*100 ms, hence 16 bits needed
    uint16_t    TempOffTime;
    uint8_t     TempOnColor;
    uint8_t     TempOffColor;
    uint8_t     PermPriority;
    uint16_t    PermOnTime;
    uint16_t    PermOffTime;
    uint8_t     PermOnColor;
    uint8_t     PermOffColor;
} TTCSPRdrLEDCfg;
...
typedef struct __attribute__((__packed__))
{
    uint8_t GPIOsBlack, MaskBlack;
    uint8_t GPIOsRed  , MaskRed  ;
    uint8_t GPIOsGreen, MaskGreen;
    uint8_t GPIOsAmber, MaskAmber;
    uint8_t GPIOsBlue , MaskBlue ;
} TTCSPLEDMap;

// Note that these match OSDP standard (except for WHITE)
#define TCSP_LEDCOLOR_BLACK     0x00
#define TCSP_LEDCOLOR_RED       0x01
#define TCSP_LEDCOLOR_GREEN     0x02
#define TCSP_LEDCOLOR_AMBER     0x03
#define TCSP_LEDCOLOR_BLUE      0x04
#define TCSP_LEDCOLOR_WHITE     0x07

// Passing configuration to Buzzer
typedef struct __attribute__((__packed__))
{
    int32_t     Volume;
    int32_t     Frequency;
...
// Flags to modify implementation of TCSP protocol
#define TCSP_MOD_IN_APP_LINK_STATUS                     0x0001  // Custom peripheral actions (in App) upon Active/Inactive transitions
#define TCSP_MOD_IN_APP_BUZZER                          0x0002  // Custom processing of Buzzer requests in App
#define TCSP_MOD_IN_APP_LEDS                            0x0004  // Custom processing of LED requests in App
#define TCSP_MOD_IN_APP_ID                              0x0008  // Custom processing of TCSP ID in App
#define TCSP_MOD_DONT_CHECK_SEQ_NUM                     0x0010  // Reader simply parrots back Host's Sequence Number
#define TCSP_MOD_DISABLE_SYS_TIMEOUT                    0x0100  // Disable Link Inactive Timeout
#define TCSP_MOD_NEW_SEQNUM_SAME_PACKET_TREAT_AS_OLD    0x0200  // If Host resends same command WITH NEW SEQNUM after receiving BUSY last
                                                                // time, treat packet as old. By default, new SeqNum = new request
#define TCSP_MOD_DISABLE_SECURE_MODE                    0x0400  // Do not use Encryption using AES128
#define TCSP_MOD_USE_SCBK_FIXED                         0x0800  // Use fixed SCBK, do not generate from UID

// TCSP Configuration Structure to be used with TCSP_Init
typedef struct __attribute__((__packed__))
{
...
typedef struct __attribute__((__packed__))
{
    uint8_t     IsHost;         // != 0 means Host
    uint8_t     UARTChannel;
    uint32_t    DataRate;       // see definitions "TCSP_DATA_RATE_####" above
    uint16_t    CustomFlags;    // bitwise definitions "TCSP_MOD_XXXXX" above
    uint8_t     TxGPIONum;      // raise GPIO bit when transmitting (invalid = no action)

    uint32_t    SerialNumber;
    uint8_t     SCBK[16];

    uint32_t    ReceiveBufSize;
    uint32_t    LargestCombinedMessageSize;
    uint32_t    TransmitDataSize;

    uint32_t    ResponseTimeout;    // how many ms before too late for Device to respond to Host
    uint32_t    SystemTimeout;      // how many ms before declaring Link Inactive (expected SeqNum = 0)

    TTCSPCapabilities   Cap;
} TTCSPConfig;

#endif

SYSFUNC(API_TCSP,  0, bool TCSP_Init(byte Standard, TTCSPConfig* Cfg))
SYSFUNC(API_TCSP,  1, void TCSP_SetAddress(byte Address))
SYSFUNC(API_TCSP,  2, bool TCSP_IsLinkActive(void))
SYSFUNC(API_TCSP,  3, bool TCSP_NewTagFound(int TagType, int IDBitCount, const byte *ID))
SYSFUNC(API_TCSP,  4, byte TCSP_ReceivePacket(byte *CommandCode, byte *RxData, int *RxDataLen, int MaxRxBytes))
SYSFUNC(API_TCSP,  5, byte TCSP_SendPacket(byte ResponseCode, byte *TxData, int TxDataLen))
SYSFUNC(API_TCSP,  6, bool TCSP_LockBuzzerUpdate(bool Lock))
SYSFUNC(API_TCSP,  7, bool TCSP_UpdateBuzzer(TTCSPRdrBuzzerCfg *Buz))
SYSFUNC(API_TCSP,  8, bool TCSP_ConfigureLED(int ID, TTCSPLEDMap *Map))
SYSFUNC(API_TCSP,  9, bool TCSP_LockLEDUpdate(bool Lock))
SYSFUNC(API_TCSP, 10, bool TCSP_UpdateLED(TTCSPRdrLEDCfg *LEDUpdate))
SYSFUNC(API_TCSP, 11, bool TCSP_UpdateStatus(byte PowerStatus, byte TamperStatus))
